...
export default async function NewSessionPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const [patient, staff, exercises, authSession] = await Promise.all([db.patient.findUnique({ where: { id }, select: { name: true } }), db.user.findMany({ select: { id: true, name: true }, orderBy: { name: "asc" } }), db.exercise.findMany({ select: { id: true, name: true, category: true }, orderBy: [{ category: "asc" }, { name: "asc" }] }), getServerSession(authOptions)]);
